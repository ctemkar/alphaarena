...
def call_model(name):
    ...
    try:
        p = LIVE_PRICES["BTC"]
        # Passing the "Bitmap" (Last 10 BTC prices)
        prompt = f"BTC Bitmap: {HISTORY[-10:]}. Reply only: BUY, SELL, or HOLD."
        if b["p"] == "ollama":
        ...
        with urllib.request.urlopen(req, timeout=5) as r:
            ...
            trade_amt = 20.0
            if "BUY" in decision and b["bal"] >= trade_amt:
                b["bal"] -= trade_amt
                b["pos"] += (trade_amt / p)
                LOGS.append(f"{name}: BUY")
            elif "SELL" in decision and b["pos"] > 0:
                b["bal"] += (b["pos"] * p)
                b["pos"] = 0
                LOGS.append(f"{name}: SELL")
    except: pass
    ...
    def do_GET(self):
        if self.path == '/data':
            ...
            if n in ARENA_DATA:
                ...
                # Auto-seed with a small position so it moves immediately
                if ARENA_DATA[n]["active"] and ARENA_DATA[n]["pos"] == 0:
                    seed = 10.0
                    ARENA_DATA[n]["bal"] -= seed
                    ARENA_DATA[n]["pos"] = seed / LIVE_PRICES["BTC"]
            elif n in SYMBOLS:
        ...
        async function update() {
            try {
                ...
                Object.entries(d.bots).forEach(([n,b])=>{
                    ...
                    if(b.active){bAct+=h; bS+=b.total;} else bIn+=h;
                });
                document.getElementById('ba').innerHTML=bAct + (bAct?`<div class="summary"><div style="font-size:24px;color:#02c076">$${bS.toFixed(2)}</div><div>ARENA TOTAL</div></div>`:'');
                ...
                document.getElementById('ca').innerHTML=cAct + (cAct?`<div class="summary"><div style="font-size:24px;color:#02c076">$${cS.toFixed(2)}</div><div>BASKET TOTAL</div></div>`:'');
